...
for index,subject in id_map.iterrows():
    try:
        ...
        processed_df = pd.concat([processed_df, pd.DataFrame({'PennEPI': [subject['PennEPI']], 'RID': [subject['RID']], 'ieeg_recon': [True]})])
        ...
        log.error(e)
        processed_df = pd.concat([processed_df, pd.DataFrame({'PennEPI': [subject['PennEPI']], 'RID': [subject['RID']], 'ieeg_recon': [False]})])
        processed_df.to_csv(Path(data_dir) / "processed_datasets.csv", index=False)
        continue
